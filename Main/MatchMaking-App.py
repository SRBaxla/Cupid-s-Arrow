...
def vectorization(df, columns, input_df):
    ...
    if column_name not in ['Gender','Bios', 'Movies','Religion', 'Music', 'Politics', 'Social Media', 'Sports']:
                
        return df, input_df
    
    # Encoding columns with respective values
    if column_name in ['Religion', 'Politics','Gender']:
...
st.title("Cupids Arrow")

st.header("Find you other half using Artificial Inteligence")
st.write("Note: All data used here is fake and generated using webscraping on online profile generator.")
...
if random_vals:
    ...
    for i in new_profile.columns[1:]:
        if i in ['Religion', 'Politics', 'Gender']:  
...
copy_profile= new_profile

# Push to start the matchmaking process
button = st.button("Click to find your Top matches!")

if button:    
    with st.spinner('Finding your Top Matches...'):
        # Vectorizing the New Data
        sex=new_profile['Gender'].values[0]
        new_profile.drop(['Gender'],axis=1,inplace=True)
        df.drop(['Gender'],axis=1,inplace=True)
        ...
        #adding the original dataset for filtering male and female profiles
        with open("refined_profiles.pkl",'rb') as fp:
            df1 = pickle.load(fp)
        common= df1[df1.index.isin(top_10_df.index.values.tolist() )]
        if sex =='male':
            filter=common.loc[common['Gender']=='female']
        else:
            filter=common.loc[common['Gender']=='male']
        for col in filter.columns:
            filter[col] = filter[col].apply(string_convert)
        df1.append(copy_profile)
        
        with open("refined_profiles.pkl",'wb') as br:
            pickle.dump(df1, br)

        # Success message   
        st.success("Found you. Top Most Similar Profiles!")    
        st.balloons()
